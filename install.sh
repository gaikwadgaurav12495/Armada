...
EXAMPLES:
  ...
  ${script} -c=ca2tvk020fa3kiiot6t0 --crn-service-name=servicename --crn-cname=prod --sos-config-path=/path/servicename0001.ovpn
  ${script} -c=ca2tvk020fa3kiiot6t0 --crn-service-name=servicename --crn-cname=prod --sos-config-path=/path/servicename0001.ovpn --uptycs=false
EOF
}
...
# DLC is no longer the part of CSUtil. Clean-up the DLC_ENABLED flag.
...
if [[ ${SERVICE_CREATED} -ne 1 ]]; then
  ...
  ibmcloud ks cluster master audit-webhook set --cluster "${CLUSTER_NAME_OR_ID}" --remote-server "${AUDIT_URL}" --ca-cert "${CA_CERT}" --client-cert "${CLIENT_CERT}" --client-key "${CLIENT_KEY}"
  SET_WEBHOOK=$?
